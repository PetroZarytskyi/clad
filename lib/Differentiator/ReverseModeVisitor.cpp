...
Expr* getArraySizeExpr(const ArrayType* AT, ASTContext& context,
  ...
  FunctionDecl* ReverseModeVisitor::CreateGradientOverload(int numAdditionalParams) {
    ...
    std::size_t totalDerivedParamsSize = m_DiffReq->getNumParams() * 2 + numAdditionalParams;
    std::size_t numOfDerivativeParams = m_DiffReq->getNumParams() + numAdditionalParams;
  ...
  ReverseModeVisitor::Derive(const FunctionDecl* FD,
    ...
    if (shouldCreateOverload) {
      gradientOverloadFD =
          CreateGradientOverload(numExtraParam);
    }
