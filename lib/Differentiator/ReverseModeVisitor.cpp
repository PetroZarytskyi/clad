...
Expr* ReverseModeVisitor::getStdInitListSizeExpr(const Expr* E) {
  ...
  StmtDiff ReverseModeVisitor::VisitCallExpr(const CallExpr* CE) {
    ...
    // arg by reference, we can request a derivative w.r.t. to this arg using the
    // forward mode.
    bool asGrad = NArgs != 1 || utils::HasAnyReferenceOrPointerArgument(FD) || isa<CXXMethodDecl>(FD);
    if (!asGrad) {
    ...
    pullbackRequest.Mode = asGrad ? DiffMode::experimental_pullback : DiffMode::experimental_pushforward;
