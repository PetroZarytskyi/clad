  ...
  namespace utils {
    ...
    bool hasMemoryTypeParams(const FunctionDecl* FD) {
      ...
      for (const ParmVarDecl* PVD : FD->parameters()) {
        // We ignore dummy params because they are not essential for the
        // analysis but sometimes have types like `enable_if<...>::type*` which
        // can trigger a reverse_forw without a necessity.
    ...
    GetDerivativeType(Sema& S, const clang::FunctionDecl* FD, DiffMode mode,
      ...
      if (mode == DiffMode::reverse_mode_forward_pass &&
        ...
        trackerTy = S.getASTContext().getLValueReferenceType(trackerTy);
