...
// CHECK: void f_1_jac(double a, double b, double c, double output[], clad::array_ref<clad::matrix<double> > _d_vector_output) {
...
// CHECK-NEXT:     * _d_vector_output[2] = (_d_vector_c * c + c * _d_vector_c) * 10 + _t3 * 0 - (_d_vector_a * a + a * _d_vector_a);
...
// CHECK: void f_3_jac(double x, double y, double z, double *_result, clad::array_ref<clad::matrix<double> > _d_vector__result) {
...
// CHECK-NEXT:     clad::array<double> _d_vector_constant(clad::array<double>(indepVarCount, 0));
...
// CHECK: void f_4_jac(double x, double y, double z, double *_result, clad::array_ref<clad::matrix<double> > _d_vector__result) {
// CHECK-NEXT:     unsigned long indepVarCount = * _d_vector__result.rows() + {{3U|3UL|3ULL}};
// CHECK-NEXT:     clad::array<double> _d_vector_x = clad::one_hot_vector(indepVarCount, {{0U|0UL|0ULL}});
// CHECK-NEXT:     clad::array<double> _d_vector_y = clad::one_hot_vector(indepVarCount, {{1U|1UL|1ULL}});
// CHECK-NEXT:     clad::array<double> _d_vector_z = clad::one_hot_vector(indepVarCount, {{2U|2UL|2ULL}});
...
// CHECK: void f_1_jac_0(double a, double b, double c, double output[], clad::array_ref<clad::matrix<double> > _d_vector_output) {
// CHECK-NEXT:     unsigned long indepVarCount = * _d_vector_output.rows() + {{3U|3UL|3ULL}};
// CHECK-NEXT:     clad::array<double> _d_vector_a = clad::one_hot_vector(indepVarCount, {{0U|0UL|0ULL}});
...
// CHECK: void f_5_jac(float a, double output[], clad::array_ref<clad::matrix<double> > _d_vector_output) {
...
int main() {
  ...
  printf("Result is = {%.2f, %.2f}", result[0][0], result[1][0]); // CHECK-EXEC: Result is = {6.00, 1.00}
}
