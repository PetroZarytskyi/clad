...
// CHECK-NEXT:     thrust::copy(std::begin(src), std::end(src), std::begin(dst));
// CHECK-NEXT:     {
...
// CHECK-NEXT:     thrust::detail::normal_iterator<device_ptr<double> > it = thrust::copy(std::begin(src), std::end(src), std::begin(dst));
